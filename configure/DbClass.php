...
define('PROJECT', 'PHPLite re-written');
define('PROJECT_URL', 'no-url');
define('VERSION', '1.0');
//-=-- ---- ---- -----=-- ---- ---- ---- ---- ---- -----==============
...
class Database
{
	...
	protected $data;
	/*
data["path"]        : path-to-/filename
data['type']        : 0=>PDO 1=>SQLite3 2=>SQLiteDatabase
data["name"]        :
data["writable"]    : t/f
data["writable_dir"]: t/f	
	*/
	...
	public function __construct($data)
	{
		...
		try{
			...
			if(
				!file_exists($this->data["path"]) && 
				!is_writable(dirname($this->data["path"]))
			){
				echo "<div class='confirm' style='margin:20px;'>";
				printf(
					"The database is not writeable", 
					htmlencode($this->data["path"]), 
					htmlencode(dirname($this->data["path"]))
				);
				exit();
			}
...
//=========================================================
	/**
	this function registers a (group of) function(s) into the 
	database
	@param $ids: single id or array of ids
	function registerUserFunction($ids)
	*/
...
//==============================================	
/**
	gets the last error depending on database type
	each of the supported databse types have their
	own error message functions,
	And this class also generates its own 'alterError'
	from when it tries to process ALTER commands unseccesfully
	getError()
*/
...
//=================================================
	...
	public function showError(){
		...
		if($classPDO){	
			// Perhps the SQLite driver for PDO is missing
			$PDOSqliteDriver = (in_array("sqlite", PDO::getAvailableDrivers() ));
		}else{
			//Or PDO class does not exist
			$PDOSqliteDriver = false;
		}

		echo "<div class='confirm' style='margin:20px;'>";
		printf('db_setup', $this->getPath());

		echo ".<br/><br/><i>".
		'chk_ext'."...<br/><br/>";
		
		echo "<b>PDO</b>: ".
		($classPDO ? 'installed' : 'not_installed')."<br/>";
		
		echo "<b>PDO SQLite Driver</b>: ".
		($PDOSqliteDriver ? 'installed' : 'not_installed')."<br/>";

		echo "<b>SQLite3</b>: ".
		($classSQLite3 ? 'installed' :'not_installed')."<br/>";

		echo "<b>SQLiteDatabase</b>: ".
		($classSQLiteDatabase ? 'installed' : 'not_installed')."<br/>";

		echo "<br/>...".'done'.".</i><br/><br/>";
...
//======================================================	
	...
	public function print_db_list()
	{
		...
		foreach($databases as $database)
		{
			...
			echo '[' . ($database['readable'] ? 'r':' ' ) . 
			($database['writable'] && $database['writable_dir'] ? 
				'w':' ' ) . ']&nbsp;';
...
//-- ---- ---- ---- ---- --
public function sqlite_surroundings_preg2(
		$name,$preg_quote=true,
		$notAllowedCharsIfNone="'\"",
		$notAllowedName=false
){
	return $this-> sqlite_surroundings_preg(
		$name,$preg_quote,
		$notAllowedCharsIfNone,
		$notAllowedName
	);
}	
private function sqlite_surroundings_preg(
		$name,$preg_quote=true,
		$notAllowedCharsIfNone="'\"",
		$notAllowedName=false
	){
		...
		if($name=="*" || $name=="+")
		{
			...
			if(
				version_compare(
					strstr(constant('PCRE_VERSION'), ' ', true),
					'8.36', '>='
				) ||
				version_compare(
					strstr(constant('PCRE_VERSION'), ' ', true), 
			...
			$nameSingle=$nameDouble=$nameBacktick=$nameSquare=$nameNo="";
		
			if($notAllowedName!==false){
				$nameSingle   = "(?!".$notAllowedName."')";
				$nameDouble   = "(?!".$notAllowedName."\")";
				$nameSquare   = "(?!".$notAllowedName."\])";
				$nameNo       = "(?!".$notAllowedName."\s)";
			}
			$nameSingle.="(?:[^\']$name+|\'\')$name".$posessive;
			$nameDouble.="(?:[^\"]$name+|\"\")$name".$posessive;
			$nameSquare.="(?:[^\]]$name+|\]\])$name".$posessive;
			$nameNo    .="[^".$notAllowedCharsIfNone."]$name";
		}else{
	...
	public function alterTable($table, $alterdefs)
	{
		...
		//work on this if and only if alterdefs exist
		if($alterdefs != '')
		{
			//does table exist?
			$resultArr = $this->selectArray(
				"SELECT sql,name,type FROM sqlite_master WHERE tbl_name = ".
				$this->quote($table)
			);
			...
			$recreateQueries = array(); //create queries for triggers and indexes
			...
			for($i=0; $i<sizeof($resultArr); $i++)
			{
				...
				if($row['type'] != 'table')
				{
					if($row['sql']!='')
					{
						// store the CREATE statements of triggers and indexes (not tables) to recreate them later
						$recreateQueries[] = $row;
					...
					$tmpname = 't'.time();
					$origsql = $row['sql'];
					$preg_remove_create_table = "/^\s*+CREATE\s++TABLE\s++".
					$this->sqlite_surroundings_preg($table)."\s*+(\(.*+)$/is";
					
					//Same query without CREATE TABLE <table-name>
					$origsql_no_create = preg_replace(
						$preg_remove_create_table, '$1', 
						$origsql, 1
					);
					...
					/*
						for some reason was not able to remove
						'CREATE TABLE' from the SQL command
					*/
					...
					//create tmpname in the image of the 
					//existing
					// table
					...
					$preg_alter_part = "/(?:DROP(?! PRIMARY KEY)|ADD(?! PRIMARY KEY)|CHANGE|RENAME TO|ADD PRIMARY KEY|DROP PRIMARY KEY)". // the ALTER command
						"(?:".
						"\s+\(".
						$this->sqlite_surroundings_preg(
							"+",
							false,
							"\"'\[`)"
						).
						"+\)".	
						// stuff in brackets (in case of ADD PRIMARY KEY)
						"|".// or
						"\s+".
						$this->sqlite_surroundings_preg(
						"+",false,",'\"\[`"
					...
					foreach($defs as $def)
					{
						...
						"(?:\s+'((?:[^']|'')+)')?".
						// $matches[6] (optional): a second column name surrounded with single quotes
						//		(the match does not contain the quotes) 
						"\s+".
						"((?:[A-Z]+\s*)+(?:\(\s*[+-]?\s*[0-9]+(?:\s*,\s*[+-]?\s*[0-9]+)?\s*\))?)\s*".
						// $matches[7]: a type name
						".*".")"."?\s*$".
						")?\s*$/i"; 
						...
						if($parse_def===false)
						{
							$this->alterError = $errormsg . 
							$lang['alter_parse_failed'];
							if($debug){
						...
						*/
						...
						$preg_columns_before =  
						// columns before the one changed/dropped (keep)
							"(?:".
							"(".			// group $2. Keep this one unchanged!
							"(?:".
							"$preg_column_definiton,\s*+".		
							// column definition + comma
						...
						switch($action)
						{
							case 'add':
								...
								$preg_pattern_add = "/^".
								$preg_create_table.   
								// the CREATE TABLE statement ($1)
								"((?:(?!,\s*(?:PRIMARY\s+KEY\s*\(|CONSTRAINT\s|UNIQUE\s*\(|CHECK\s*\(|FOREIGN\s+KEY\s*\()).)*)". 
							...
							case 'change':
								...
								$newSQL=="") // pattern did not match, so column removal did not succed
								{
	...
	public function getTableNames(){
		...
		foreach ($Stuffs as $key => $value) {
			...
			if ('table' ==$value['type']) {
				if(
					"sqlite_sequence" != $value['tbl_name'] &&
					"android_metadata"!= $value['tbl_name'] &&
					"sqlite_master"   != $value['tbl_name'] 
				){
					$tables[] = $value['tbl_name'];
				}
			}
		}
	...
	function getColumnNames($Table){
		...
		foreach ($res2[3] AS $key => $value) {
			if($value !=""){
				$outcome[]= $this->enQuote($this->deQuote($value));
			}else{
				$outcome[]= $this->enQuote($this->deQuote($res2[8][$key]));
			}
		}//fore

		foreach ($outcome as $key => $value) {
			$outcome[$key] = $this->deQuote($value);
		}

		return $outcome;
	}
	
	private function enQuote($str){
