...
if( 
	...
	if( is_file($_POST["filename"])	){
		...
		foreach ($DD as $key => $value) {
			# code...
			$Src = str_replace($value, '<script src ="'.$QQ[$key].
...
if( is_file($_POST["filename"])	){
	...
	$Src = file_get_contents($_POST["filename"]);
	$res = array();
	preg_match_all('/\<script\b([^>]*)>/', $Src, $res);
	...
	foreach ($res[1] as $key => $value) {
		# code...
		$res2 =	array();
		preg_match_all('/src ?= ?"([^"]+)"/', $value,$res2);

		if( isset($res2[1][0]) ){
			$AllSrc[] =array('string'=>"&lt;script $value &gt;", "src"=> $res2[1][0]);
		}
	}
